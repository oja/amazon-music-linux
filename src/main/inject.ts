...
var __am = {
    getMusicTitle: function () {
        ...
        (document.getElementsByClassName('button nextButton icon-fastForward transportButton')[0] as HTMLElement).click()
    }, previousTrack: function () {
        (document.getElementsByClassName('button previousButton icon-fastBackward transportButton')[0] as HTMLElement).click()
    }, getSongImage: function () {
        return (document.getElementsByClassName('renderImage')[0] as HTMLImageElement).src
        ...
        try {
            ...
            if (lines != undefined) {
                if (__am.highlighted == null || __am.highlighted == undefined) {
                    ...
                    __am.sendToElectron((__am.highlighted as HTMLElement).innerHTML)

                }

                if (__am.highlighted != undefined && document.getElementById(__am.highlighted.id).classList.item(1) != 'highlighted') {
                    __am.highlighted = document.getElementById(__am.highlighted.id).nextSibling as HTMLElement
    ...
    }, sendToElectron: function (text: String) {
        ...
        setTimeout(() => {
            (document.getElementsByClassName('playbackControls')[0].children[1] as HTMLElement).onclick = () => {
        ...
        setTimeout(() => {
            (document.getElementsByClassName('playbackControls')[0].children[2] as HTMLElement).onclick = () => {
        ...
        setTimeout(() => {
            (document.getElementsByClassName('playbackControls')[0].children[0] as HTMLElement).onclick = () => {
